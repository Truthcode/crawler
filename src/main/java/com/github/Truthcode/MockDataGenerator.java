...
public class MockDataGenerator {
    private static void mockData(SqlSessionFactory sqlSessionFactory, int howMany) {
        try (SqlSession session = sqlSessionFactory.openSession(ExecutorType.BATCH)) {
            List<News> currentNews = session.selectList("Com.github.Truthcode.MockMapper.selectNews");
            ...
            try {
                while (count-- > 0) {
                    ...
                    session.insert("Com.github.Truthcode.MockMapper.insertNews", newsToBeInserted);
